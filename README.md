...
Пример запуска отредактированного скрипта:

sudo python3 scapy_xss_analyzer.py --analyze traffic2.pcap
Анализ трафика из файла: traffic2.pcap

Найдено HTTP-запросов: 5

Найдено HTTP-ответов: 6

Анализ HTTP-запросов на XSS-полезные нагрузки:

Обнаружена XSS-полезная нагрузка:

  Payload: document.cookie

  Фрагмент запроса:

GET /408086229062912822940580099856782328160/newsnippet2?snippet=%3Cimg+src%3Dx+onerror%3D%22alert%28%27test+xss.+Cookie%3A+%27+%2B+document.cookie%29%22%3E+ HTTP/1.1
...
Признаки отражённого XSS не обнаружены
